...
on public.summer_bingo_boards for select
to authenticated
using ((select auth.uid()) = user_id);

create policy "Users can insert their own summer bingo board"
on public.summer_bingo_boards for insert
to authenticated
with check ((select auth.uid()) = user_id);

create policy "Users can update their own summer bingo board"
on public.summer_bingo_boards for update
to authenticated
using ((select auth.uid()) = user_id)
with check ((select auth.uid()) = user_id);

create policy "Users can delete their own summer bingo board"
on public.summer_bingo_boards for delete
to authenticated
using ((select auth.uid()) = user_id);

grant select, insert, update, delete on public.summer_bingo_boards to authenticated;
