import {
  ...
  type Extensions,
  type JSONContent,
  type Mark as MarkExtension,
  type MarkConfig,
  mergeAttributes,
  type Node as NodeExtension,
  type NodeConfig,
} from "@tiptap/core";
import {
...
import { type DOMOutputSpec, type Mark } from "@tiptap/pm/model";
...
import { resolveExtensions } from "../utils";
...
export function mapNodeExtensionToReactNode<T>(
  ...
  options?: Partial<
    Pick<TiptapStaticRendererOptions<T, Mark, Node>, "unhandledNode">
  >
    ...
    ({ node, children }) => {
      try {
        ...
        throw new Error(
          `[tiptap error]: Node ${
            extension.name
          } cannot be rendered, it's "renderToHTML" method threw an error: ${
            (e as Error).message
          }`,
          { cause: e }
        );
...
export function mapMarkExtensionToReactNode<T>(
  ...
  extensionAttributes: ExtensionAttribute[],
  options?: Partial<
    Pick<TiptapStaticRendererOptions<T, Mark, Node>, "unhandledMark">
  >
...
export function renderToElement<T>({
  ...
  renderer: (
    options: TiptapStaticRendererOptions<T, Mark, Node>
  ) => (ctx: { content: Node; extensions?: Extensions }) => T;
  domOutputSpecToElement: DomOutputSpecToElement<T>;
  mapDefinedTypes: {
