...
export async function GET() {
  const authError = await getPaletteAuthError();
  if (authError) {
    return authError;
  }
  ...
  if (error) {
    console.error("Failed to load summer palette board", {
...
export async function POST(req: NextRequest) {
  ...
  const sessionUser = await getSessionUser();
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.from("summer_palette_boards").upsert(
    {
  ...
  if (error) {
    console.error("Failed to save summer palette board", {
      userId: sessionUser!.id,
      error,
    });
    return NextResponse.json(
...
async function getPaletteAuthError() {
  const sessionUser = await getSessionUser();
